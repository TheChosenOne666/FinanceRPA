...
| **状态** | ✅ 已完成（2026-08-02）。实现前端审批中心完整页面。最终落地：(1) **新建 `api/approval.ts`**：4 个 API 函数封装（listApprovals 分页查询 / getApprovalDetail 详情 / approveApproval 通过 / rejectApproval 拒绝），对齐 ApprovalController 4 个端点，approverId 由后端从登录上下文获取前端无需传<br>(2) **`types.ts` 追加审批类型**：ApprovalStatus（PENDING/APPROVED/REJECTED/TIMEOUT）/ ApprovalRoute（auto/department/compliance）/ ApprovalRequestVO（对齐后端 VO 全字段）/ ApprovalQueryRequest / ApprovalActionRequest<br>(3) **新建 `ApprovalStatusBadge.tsx`**：独立审批状态徽章组件（PENDING 金色脉动 / APPROVED 绿 / REJECTED 红 / TIMEOUT 深红），独立于 StatusBadge 避免扩展 TaskStatus 联合类型破坏现有调用方<br>(4) **新建 `ApprovalCenter.tsx` 审批中心主页**：①Tab 切换"待审批"（固定查 PENDING，前端按风险等级降序排序 critical 优先）/"历史记录"（默认全部，支持状态筛选 APPROVED/REJECTED/TIMEOUT）②筛选栏：风险等级 + 审批路由 + 状态（仅历史记录 Tab）③表格列：审批单 ID / 任务 ID / 任务目标 / 风险等级（tag-risk-* 标签）/ 审批路由 / 状态（ApprovalStatusBadge）/ 创建时间 / 操作④审批详情弹窗：信息网格（审批单 ID / 任务 ID / 风险等级 / 路由 / 状态 / 创建时间 / 超时截止 / 审批时间）+ 任务目标（从 requestPayload JSON 解析 goal）+ 风险判断理由（橙色高亮框）+ 审批结果（终态展示通过/拒绝理由）+ 操作表单（PENDING 状态：理由 textarea + 拒绝按钮 btn-danger + 通过按钮，拒绝时理由必填）⑤自动轮询：待审批 10s / 历史记录 30s ⑥操作成功后 invalidateQueries 刷新列表<br>(5) **`Icons.tsx` 新增 IconApproval**：文档 + 对勾图标（区别于 IconShield 已用于"接管"）<br>(6) **`router.tsx` 新增路由** `/approvals` → ApprovalCenter<br>(7) **`RootLayout.tsx` 顶部导航新增"审批"按钮**（IconApproval，位于"接管"与"监控"之间）<br>(8) **`glass.css` 新增审批样式**：approval-tabs/approval-tab（Tab 切换栏）+ approval-detail-meta（详情信息网格）+ approval-detail-text（文本展示框）+ approval-detail-reasoning（风险理由橙色高亮框）+ btn-danger（拒绝按钮红色变体）<br>(9) **设计决策**：①新建独立 ApprovalStatusBadge 而非扩展 StatusBadge，避免 TypeScript 联合类型扩展风险 ②复用 .tasks-* 表格/布局样式类（通用列表样式）+ .tag-risk-* 风险标签，最小化新增 CSS ③requestPayload 为 TaskTriggerRequest JSON 字符串，前端 JSON.parse 提取 goal 展示任务目标 ④"截图"项未实现：后端 ApprovalRequestVO 无截图字段（审批在任务执行前进行，PENDING_APPROVAL 状态任务未执行无截图） |
| **测试覆盖** | TypeScript 类型检查通过（tsc -b 零错误）+ vite build 成功（802 modules transformed，13.98s）；无前端单元测试框架（项目未配置 Vitest），通过类型检查 + 构建验证 |
| **UI 原型对齐** | ✅ 已完成（2026-08-04）。对齐原型 `05-approval-center.html`，从"单栏表格+弹窗"重构为"双栏工作台"布局。最终落地：(1) **布局重构**：`ApprovalCenter.tsx` 从单栏表格+弹窗模式重构为 `page-grid grid-1-2` 双栏工作台（左 1fr 审批卡片列表 + 右 2fr 固定详情面板），`.approval-workbench` 单独设置 `grid-template-columns: 1fr 2fr` 避免被全局 `grid-1-2` 媒体查询覆盖，仅 ≤768px 转单列 (2) **Tab 结构**：从 2 个 Tab（待审批/历史记录）改为 3 个（待我审批[计数徽章] / 我发起的 / 历史），待我审批 Tab 显示 PENDING 计数，pill 样式对齐原型 (3) **面包屑**：新增"首页 / 合规 / 审批中心"面包屑 (4) **左侧审批卡片**：风险色左边框（critical 深红/high 红/medium 黄/low 绿）+ 任务名 + 任务 ID + 审批流（申请人→我）+ 倒计时（30min 内 danger 红 / 1h 内 warning 黄 / 其余 normal，每秒刷新）(5) **右侧固定详情面板**：标题区（任务名+风险徽章+任务 ID）+ 元信息网格（4 列：风险等级/申请人/触发时间/剩余时间）+ 审批流程链（申请人→我+路由标注）+ 风险原因列表（红色调 IconAlert）+ 任务参数表（从 requestPayload.params 解析）+ 执行截图占位（before/after 双列）+ 审批操作区（拒绝 btn-danger + 批准 btn-success + 拒绝理由 textarea 200 字计数）(6) **历史记录终态展示**：右侧面板显示审批结果（状态徽章+审批时间+通过/拒绝理由）替代操作区 (7) **倒计时工具函数** `calcCountdown(timeoutAt)`：实时计算剩余时间，danger/warning/normal 三级配色 (8) **任务参数解析** `parseTaskParams(payload)`：从 requestPayload JSON 解析 params 对象为键值对表格 (9) **mockServer 增强**：6 条审批数据（3 PENDING + 3 历史终态），含 riskReasoning 风险理由 + requestPayload 任务参数 + userId 筛选支持 (10) **types.ts 扩展**：ApprovalQueryRequest 新增 `userId?` 字段用于"我发起的"Tab 筛选 (11) **glass.css 新增样式**：approval-page/header/tabs/tab-count/refresh-btn/workbench/list-col/card（含 4 级风险边框）/countdown（3 级配色）/detail-panel/meta-grid（4 列）/flow-chain/reason-list/screenshot-grid/action-bar/reason-input/result-block 等，响应式 1024px 元信息转 2 列 + 768px 工作台转单列 |
...
| **描述** | 1. 通道：企业微信群机器人 + 钉钉群机器人 Webhook<br>2. 模板：审批待处理 / 审批超时 / 任务失败 / NEEDS_HUMAN / 风险升级<br>3. 配置：Webhook URL 从环境变量读取<br>4. API：`GET /api/v1/notification/channels` / `POST /api/v1/notification/test`<br>5. 扩展：集成审批触发 + Fallback 机制 + Redis 重试队列 + NotificationAttempt 持久化 + 重试队列统计 API + 前端通知中心页面 |
| **验收标准** | 通知发送到指定群；模板渲染正确；测试接口可用；主通道失败自动 Fallback；失败任务入 Redis 重试队列；所有发送尝试持久化到 rpa_notification_attempt 表 |
| **状态** | ✅ 已完成（2026-08-02 ~ 2026-08-03）。第一阶段实现通知模块最小集（通道 + 模板 + 配置 + 2 个 API）；第二阶段（2026-08-03）扩展完成审批触发 + Fallback + Redis 重试队列 + NotificationAttempt 持久化 + 重试队列统计 API + 前端通知中心页面。最终落地：<br><br>**【第一阶段 · 基础设施】**(1) **新建 `notification/` 包结构**：constant / enums / config / channels / templates / dto(request+response) / service(impl) / controller 八个子包，对齐项目既有模块（approval / agent）的分层风格<br>(2) **常量与枚举**：`NotificationConstant`（通道值 wecom/dingtalk + 5 种模板类型 + 钉钉加签算法 HmacSHA256 + 重试队列 Redis Key `notification:retry:queue` + 最大重试次数 3 + 重试扫描 cron `0 */5 * * * ?`）/ `NotificationChannelEnum`（WECOM/DINGTALK 含 getEnumByValue）/ `NotificationTemplateEnum`（APPROVAL_PENDING/APPROVAL_TIMEOUT/TASK_FAILED/NEEDS_HUMAN/RISK_ESCALATION 含 getEnumByValue）<br>(3) **配置**：`NotificationProperties`（@ConfigurationProperties prefix=notification）+ `NotificationWebClientConfig`（独立 WebClient Bean，连接 5s / 读写 10s，与 aiWebClient 隔离避免通知失败影响 AI 调用链）；`application.yml` 新增 `notification.wecom.webhook-url` / `notification.dingtalk.webhook-url` / `notification.dingtalk.secret` 三项，全部通过环境变量 `WECOM_WEBHOOK_URL` / `DINGTALK_WEBHOOK_URL` / `DINGTALK_SECRET` 注入<br>(4) **通道实现**：`NotificationChannel` 接口（getChannel / isConfigured / send）+ `WeComChannel`（POST Webhook，body `{"msgtype":"markdown","markdown":{"content":"..."}}`，按响应 `errcode:0` 判定成功）+ `DingTalkChannel`（POST Webhook，body 含 `markdown.title` 与 `markdown.text`；secret 非空时启用加签：`sign=URLEncode(Base64(HmacSHA256(timestamp+"\n"+secret, secret)))` 拼接 `&timestamp=...&sign=...`）<br>(5) **模板渲染**：`NotificationTemplateRenderer` 按 `NotificationTemplateEnum` switch 分发到 5 个 render 方法；每个模板用 Java 21 text block 生成 markdown 正文（标题 `## xxx` + 引用 `> ...` + 字段列表）；参数从 `Map<String,Object>` 安全提取，缺失字段用 "-" 占位<br>(6) **DTO**：`NotificationMessage` / `NotificationTestRequest` / `ChannelVO` / `NotificationSendResultVO`（含 success/failure 静态工厂方法）<br>(7) **Service**：`NotificationService` 接口（listChannels / send / test）+ `NotificationServiceImpl` 持有 `List<NotificationChannel>`（Spring 自动收集）<br>(8) **Controller**：`NotificationController` 路径 `/notification`；GET /channels + POST /test<br><br>**【第二阶段 · 扩展能力】**(9) **集成审批触发**：`ApprovalServiceImpl` 新增 `@Resource NotificationService` 依赖；`createApproval` 在 Pub/Sub 发布后调用 `notificationService.dispatch(APPROVAL_PENDING, ...)` 触发"审批待处理"通知；`processTimeoutApprovals` 在审批单标记 TIMEOUT 后调用 `notificationService.dispatch(APPROVAL_TIMEOUT, ...)` 触发"审批超时告警"通知（每个超时审批触发一次）；通知失败不影响主流程（try/catch + 日志告警）<br>(10) **Fallback 机制 + Redis 重试队列**：新增 `dispatcher/NotificationDispatcher.java`（持有 channels 列表 + RedissonClient + ObjectMapper）：①`dispatch(template, params, orgId, userId, taskId, approvalId)` 方法按通道顺序尝试发送：主通道（WeCom）失败 → Fallback 到次通道（DingTalk）→ 全部失败则 `enqueueRetry` 入 Redis List 队列 `notification:retry:queue`（序列化为 `NotificationRetryTask` JSON：含 template/params/orgId/userId/taskId/approvalId/retryCount/nextRetryAt）②每次尝试都调用 `attemptService.recordAttempt` 持久化发送结果（成功/失败/重试）<br>(11) **NotificationRetryScheduler**：`@Scheduled(cron = "0 */5 * * * ?")` + `@SchedulerLock(name = "notification:retry:scan", lockAtMostFor = "PT4M", lockAtLeastFor = "PT1M")` 实现 ShedLock 分布式锁单节点执行（每 5 分钟扫描）；从 Redis 队列 `lpop` 取出任务 → 渲染模板 → 调用通道发送 → 成功移除 / 失败 retryCount+1 重新 `rpush` 入队；retryCount >= MAX_RETRY_TIMES（3）则告警计数 + 不再重试（待人工介入）<br>(12) **NotificationAttempt 持久化**：①`entity/NotificationAttemptEO`（id + orgId + templateType + channel + taskId + approvalId + success + errorMessage + rawResponse + retryCount + attemptTime + createTime，雪花主键 + 逻辑删除）②`V16__create_notification_attempt_table.sql` 建表（含 org_id 索引 + template_type 索引 + task_id 索引 + 模板/通道/成功字段索引）③`mapper/NotificationAttemptMapper` ④`service/NotificationAttemptService` + `impl/NotificationAttemptServiceImpl`：`recordAttempt` 写入 + `countAttempts(startTime, endTime)` 计数 + `calculateSuccessRate(startTime, endTime)` 成功率（避免除零，无记录返回 0.0）⑤`TenantConstant.IGNORED_TABLES` 已加入 `rpa_notification_attempt`（与 rpa_skill_meta / rpa_workflow_template 一致，全局共享表）<br>(13) **重试队列统计 API**：扩展 `NotificationService` 新增 `dispatch(...)` / `getRetryQueueSize()` / `getRetryStats()` 三个方法；扩展 `NotificationController` 新增 `GET /retry/queue`（返回 long 队列长度）+ `GET /retry/stats`（返回 `RetryQueueStatsVO`：queueSize + totalAttempts + successCount + failureCount + successRate + alertCount）；alertCount 暂等于 queueSize（超过最大重试次数的告警计数，后续扩展为独立计数器）<br>(14) **前端通知中心**：①`api/types.ts` 追加通知类型：NotificationChannelType / NotificationTemplateType / ChannelVO / NotificationSendResultVO / NotificationTestRequest / RetryQueueStatsVO ②新建 `api/notification.ts`：4 个 API 函数封装（listChannels / sendTestNotification / getRetryQueueSize / getRetryStats）对齐 NotificationController 4 个端点 ③新建 `routes/notification/NotificationCenter.tsx`：a. 顶部统计卡片网格（4 张卡片：队列待重试-蓝色顶边框 / 总尝试次数-蓝色 / 成功率-绿色 / 告警数-红色，每张含标题 + 大字号数值 + 辅助说明）b. 通道配置状态板块（卡片网格，已配置通道绿色描边 + 浅绿背景 + IconCheck，未配置灰色 + IconClose）c. 测试发送表单（通道下拉 + 模板类型下拉 + JSON 参数 textarea 等宽字体 + 发送按钮 IconSend）d. 切换模板自动填充对应默认 JSON 参数示例（5 种模板各对应一组示例参数）e. 发送结果展示（成功绿色描边 + IconCheck + 通道响应 pre 块；失败红色描边 + IconClose + 错误信息 pre 块）f. 自动轮询 30s 刷新统计 + 通道状态（refetchInterval）g. 手动刷新按钮（IconRefresh）h. JSON 参数校验（必须是对象非数组）④`router.tsx` 新增 `/notification` 路由 ⑤`RootLayout.tsx` 顶部导航新增"通知"按钮（IconBell，位于"审批"与"监控"之间）⑥`Icons.tsx` 新增 IconBell（铃铛）+ IconSend（纸飞机）⑦`glass.css` 新增通知样式：notification-stats-grid（自适应 4 列网格）+ notification-stat-card（基础卡片 + 4 种彩色顶边框变体 info/success/warning/danger）+ notification-stat-title/value/hint（标题/大数值/辅助说明）+ notification-section / -section-title（板块容器 + 图标标题）+ notification-channels / -channel-card / -channel-configured / -channel-label / -channel-status（通道卡片网格 + 已配置绿色描边）+ notification-test-form / -form-row / -params-input（等宽字体 JSON 输入框）+ notification-result / -success / -failure / -header / -channel / -body / -pre（发送结果展示，成功绿色失败红色描边） |
| **测试覆盖** | 第二阶段扩展后新增/修改测试全部通过：(1) **NotificationServiceImplTest** 扩展 3 个用例：dispatch 委派 Dispatcher / getRetryQueueSize 返回 Redis 队列长度 / getRetryStats 聚合队列长度+总尝试次数+成功率（用 `doReturn().when()` 解决 Mockito 泛型推断问题）(2) **ApprovalServiceImplTest** 扩展：createApproval_highRisk 新增 verify notificationService.dispatch(APPROVAL_PENDING) / createApproval_criticalRisk 不验证通知（critical 走另一路径）/ processTimeoutApprovals_HasTimeout 新增 verify notificationService.dispatch(APPROVAL_TIMEOUT, times(2))；用 `any()` 匹配 approvalId（mock 测试中 MyBatis 雪花赋值不生效，id 为 null）(3) **前端构建验证**：tsc -b 类型检查通过 + vite build 成功（804 modules transformed，5.72s）(4) **浏览器联调**：内置浏览器打开 http://localhost:5176/ → admin/admin123 登录 → 顶部导航点击"通知"进入 /notification → 页面渲染正常（统计卡片 4 色顶边框 / 通道卡片 / 测试发送表单完整）→ 切换模板类型自动填充默认 JSON 参数 → 无 401/API 报错 |
| **三端联调** | ✅ 已完成（2026-08-02）。M6 全模块三端联调测试（前端 + 后端 + AI 端），15 项测试全部通过：<br>**【M6.5 前端审批中心】** 4/4 ✓：GET /approvals 分页查询（status/route/riskLevel/taskId 筛选）/ GET /approvals/{id} 详情 / POST /approvals/{id}/approve / POST /approvals/{id}/reject；前端待审/历史 Tab + 详情弹窗 + 通过/拒绝操作均正常<br>**【M6.6 通知中心】** 4/4 ✓：GET /notification/channels（返回 dingtalk+wecom 均 configured=false）/ GET /notification/retry/queue（队列长度正确）/ GET /notification/retry/stats（queueSize/totalAttempts/successRate/alertCount 字段齐全）/ POST /notification/test（返回业务错误码 50001 + message="通道未配置 Webhook URL: WECOM"）；前端 4 张统计卡片数据正确展示；错误提示正确透传后端 message<br>**【Java↔Python 内部通道】** 7/7 ✓：Python→Java 4 项（GET /internal/approvals/99001/result 带 token 返回 APPROVED / 无 token HTTP 401 / 错误 token HTTP 401 / POST /internal/audit/logs 写入数据库成功）+ Java→Python 3 项（GET /api/v1/ai/health / GET /api/v1/ai/skills 返回 Skill 元数据 / POST /api/v1/ai/risk/judge 返回 finalRiskLevel=high + approvalRoute=department，容错机制工作正常） |
| **联调修复** | ✅ 已完成（2026-08-02）。联调测试发现 4 个问题并全部修复：<br>**(1) Python 端缺少 X-Internal-Token 鉴权中间件**（违反项目约束"Java-Python 服务认证：Use X-Internal-Token header with shared secret"）：在 `finance-ai/app/main.py` 新增 `InternalTokenMiddleware`（继承 BaseHTTPMiddleware），拦截 `/api/v1/ai/*` 路径校验 X-Internal-Token Header（排除 /health 公开路径），鉴权失败返回 HTTP 401 + JSON body；Skyvern 原生路由 /v1、/api/v1/tasks、/api/v2/* 由 Skyvern 自身鉴权不拦截；验证：无 token → HTTP 401 ✓ / 带正确 token → HTTP 200 ✓ / 无 token 访问 /health → HTTP 200 ✓<br>**(2) InternalTokenInterceptor 401 响应 body 未写出**：`InternalTokenInterceptor.java` 中 `response.getWriter().write()` 后未 `flush()`，导致 body 可能被后续 filter 吞掉；修复：补充 `response.getWriter().flush()` + try-catch IOException 日志告警；验证：无 token 请求返回 HTTP 401 + Body `{"code":40300,"data":null,"message":"内部 API 鉴权失败"}` ✓<br>**(3) 前端 RetryQueueStatsVO 类型与后端 Long→String 序列化不一致**：后端 `JsonConfig` 将所有 Long 字段序列化为 String（防 JS 精度丢失），前端类型定义为 `number` 会类型不匹配；修复：`api/types.ts` 中 RetryQueueStatsVO 字段类型改为 `string \| number` 兼容；`NotificationCenter.tsx` 数学运算处显式 `Number()` 转换（`Number(stats.totalAttempts) > 0` / `Math.round(Number(stats.successRate) * 100)`）；验证：tsc -b 类型检查通过 + 浏览器渲染统计卡片正常 ✓<br>**(4) LLM api_key 配置链路问题**：docker-compose.yml 给 finance-ai 容器只配了 `VOLCENGINE_API_KEY`（Skyvern 视觉决策用），未配 `LLM_API_KEY`（finance-ai 自有 risk_judge 用），导致 litellm 调用报错 "OPENAI_API_KEY environment variable"；修复：docker-compose.yml 给 finance-ai 新增 `LLM_PROVIDER=volcengine` + `LLM_API_KEY=${VOLCENGINE_API_KEY:-}` + `LLM_BASE_URL=https://ark.cn-beijing.volces.com/api/v3` + `LLM_MODEL=volcengine/doubao-seed-evolving`，复用火山方舟豆包模型；根目录 `.env` 同步更新 LLM 配置注释与默认值；验证：POST /api/v1/ai/risk/judge 真实调用豆包模型成功，返回中文推理理由 "任务涉及转账高风险操作，金额为50,000元，达到大额资金阈值..." + finalRiskLevel=high + approvalRoute=department ✓ |
...
| **状态** | ✅ 已完成（2026-08-03）。按系统设计 6.4.4 + ADR-004 实现，Python 不直接写 MinIO，统一走 Java 转发。最终落地：<br>(1) **application.yml 配置**：新增 `minio.*` 配置块（endpoint/access-key/secret-key/bucket-prefix/presign-expiry-hours/retention-days，均支持环境变量占位符）；新增 `spring.servlet.multipart` 配置（max-file-size=10MB / max-request-size=15MB，适配 PNG 截图 2-5MB 体积）<br>(2) **MinioProperties**（`audit/config/`）：`@ConfigurationProperties(prefix="minio")` 绑定配置项 + `@Bean MinioClient` 构建（endpoint + credentials）<br>(3) **AuditConstant 扩展**：截图阶段常量（before/after）、文件扩展名（.png）、路径分隔符、清理调度器常量（cron=每日凌晨3点 / lockName / lockAtMost=30分钟 / lockAtLeast=10秒）<br>(4) **MinioStorageService 接口 + 实现**（`audit/storage/`）：3 个核心方法 —— uploadScreenshot（multipart 文件上传到 MinIO，返回对象路径+预签名URL）/ presignUrl（为已存储对象生成预签名URL）/ deleteExpiredObjects（遍历所有审计 bucket，删除超过保留期的对象）；懒加载 bucket（首次上传时 bucketExists 检查 + makeBucket 创建）；路径规则 `{yyyy-MM-dd}/{taskId}/{stepIndex}_{phase}.png`；预签名有效期由 `minio.presign-expiry-hours` 配置（默认1小时）；清理逻辑遍历 listBuckets + listObjects(recursive) + 按 lastModified 判断 + removeObject，仅处理 `finrpa-audit-` 前缀的 bucket<br>(5) **ScreenshotUploadVO**（`audit/dto/response/`）：返回 objectPath + presignUrl，Python 上传后拿到 URL 再填入 /internal/audit/logs 的 beforeScreenshotUrl/afterScreenshotUrl<br>(6) **InternalScreenshotController**（`audit/controller/`）：POST /internal/screenshots（multipart/form-data，@RequestPart MultipartFile file + @RequestParam orgId/taskId/stepIndex/phase），X-Internal-Token 鉴权由 InternalTokenInterceptor 拦截<br>(7) **AuditScreenshotCleanupScheduler**（`audit/scheduler/`）：@Scheduled(cron=每日凌晨3点) + @SchedulerLock（复用 M6.4 ShedLockConfig，锁名 audit-screenshot-cleanup），调用 deleteExpiredObjects 清理 90 天前对象，try-catch 容错不向外抛异常 |
| **测试覆盖** | ✅ 全量 525 个测试通过（含 M7.2 新增 3 个测试类共 19 个用例）：(1) **MinioStorageServiceImplTest** 14 个用例：uploadScreenshot 成功（bucket 已存在直接上传 / bucket 不存在自动创建 / contentType 为空默认 image/png / 路径格式 {date}/{taskId}/{stepIndex}_{phase}.png 验证）+ 参数校验（文件空 / orgId 空 / taskId 空 / stepIndex 负 / phase 非法 / putObject 失败）+ presignUrl（成功 / orgId 空 / 路径空 / 生成失败）+ deleteExpiredObjects（删除过期保留未过期 / 无过期返回0 / 非审计 bucket 跳过 / listBuckets 异常返回0不抛）(2) **InternalScreenshotControllerTest** 2 个用例：before 阶段上传成功 + after 阶段上传成功（MockMvc multipart 验证 JSON 响应） (3) **AuditScreenshotCleanupSchedulerTest** 3 个用例：正常调用 deleteExpiredObjects / 服务异常时不抛出（调度器容错）/ 无过期对象正常返回 |
...
| **规模** | M |
| **前置依赖** | M7.1、M7.2 |
| **产出物** | `app/audit/` 模块（reporter.py + cache.py + schemas.py + __init__.py） |
...
| **状态** | ✅ 已完成（2026-08-03）。按系统设计 6.4.2 实现完整审计闭环：截图上传 → 元数据上报 → 失败本地缓存 → 恢复批量上报。最终落地：<br>(1) **`app/audit/schemas.py`**：①`AuditLogPayload`（Pydantic BaseModel）对齐 Java `AuditLogCreateRequest` 完整 18 字段，按系统设计 6.4.1 六大维度组织 —— 基本信息（taskId/orgId/departmentId/businessLineId/userId）/ 操作信息（actionType/targetElement/pageUrl/actionParams/executionResult/errorMessage）/ 风险信息（riskLevel/approvalId）/ 时间信息（startedAt/completedAt/durationMs）/ 截图信息（beforeScreenshotUrl/afterScreenshotUrl）/ LLM 信息（llmModel/llmTokensUsed/llmCost）；全部字段使用 camelCase alias 对齐 Java 字段名，`populate_by_name=True` 支持两种命名风格，`extra="ignore"` 忽略未知字段；除 taskId/orgId/actionType 必填外其余可选 ②`ScreenshotUploadResult`：对齐 Java `ScreenshotUploadVO`（objectPath + presignUrl）<br>(2) **`app/audit/cache.py`**（`AuditCache` 类）：Java 不可用时将待上报项序列化到本地 JSON 文件，进程重启不丢失；存储格式每个待上报项一个 JSON 文件，文件名 `{UTC时间戳}_{uuid8}.json`，内容含 type（audit_log/screenshot）+ payload + cached_at；4 个核心方法：`save(item_type, payload)` 写入（自动创建目录 + 异常容错返回 None）/ `load_all()` 按文件名时间序加载（跳过非 JSON 文件 + 跳过损坏 JSON + 附加 filepath 字段）/ `remove(filepath)` 删除已成功上报的缓存文件（不存在时静默处理）/ `count()` 返回缓存文件数；缓存目录从 `Settings.audit_cache_dir` 读取（默认 `/tmp/finrpa-audit-cache`）<br>(3) **`app/audit/reporter.py`**（`AuditReporter` 类）：封装完整审计闭环，4 个核心方法：①`upload_screenshot(org_id, task_id, step_index, phase, image_data)` 单张截图上传，Java 成功返回预签名 URL，失败/异常时缓存到本地（base64 编码 image_data）返回 None ②`report_audit_log(payload)` 上报审计元数据，Java 成功返回 True，失败/异常时缓存 `payload.model_dump(by_alias=True, exclude_none=True)` 返回 False ③`report_step(...)` 一步操作完整闭环：先上传 before 截图拿 URL → 再上传 after 截图拿 URL → 最后上报元数据（含前后 URL）；截图失败不阻断主流程（URL 为 None 继续）；参数覆盖全部 18 字段（org_id/task_id/step_index/action_type/before_screenshot/after_screenshot/started_at/completed_at/duration_ms/page_url/action_params/execution_result/error_message/risk_level/approval_id/department_id/business_line_id/user_id/llm_model/llm_tokens_used/llm_cost）④`flush_cache()` 恢复后批量上报：遍历缓存目录逐项重试，screenshot 类型 base64 解码后调 `java_client.upload_screenshot`，audit_log 类型 `AuditLogPayload.model_validate` 反序列化后调 `java_client.report_audit_log`；成功则删除缓存文件，失败保留待下次 flush；截图缓存重试不再次缓存避免无限增长<br>(4) **`app/audit/__init__.py`**：导出 `AuditReporter` 与 `AuditLogPayload`<br>(5) **`JavaBackendClient` 扩展**：①`upload_screenshot(org_id, task_id, step_index, phase, image_data)` 对齐 M7.2 `InternalScreenshotController` 参数 —— multipart/form-data，`files={"file": (filename, image_data, "image/png")}` + `data={orgId, taskId, stepIndex, phase}`；解析 Java `BaseResponse<ScreenshotUploadVO>` 封装（`data.presignUrl`），兼容直接返回 `presignUrl` 的情况 ②`report_audit_log(payload)` 对齐 M7.1 `AuditLogCreateRequest` 完整结构 —— 接受 `AuditLogPayload` 对象或 dict，统一 `model_dump(by_alias=True, exclude_none=True)` 序列化后 POST `/api/internal/audit/logs`<br>(6) **`config.py` 扩展**：`Settings` 新增 `audit_cache_dir: str = "/tmp/finrpa-audit-cache"` 配置项（M7.3 审计缓存目录，Java 不可用时持久化待上报项） |
| **测试覆盖** | ✅ 25 个单元测试全部通过（Python 3.13.12 + pytest 9.1.1 + pytest-asyncio 1.4.0）：(1) **test_audit_cache.py** 11 个用例：save+load_all 往返 / 自动创建目录 / 多文件按时间序加载 / remove 删除文件 / remove 不存在文件不抛异常 / 空目录返回空列表 / 目录不存在返回空列表 / count 返回文件数 / count 忽略非 JSON 文件 / 截图含 base64 二进制数据往返 / 损坏 JSON 文件被跳过 (2) **test_audit_reporter.py** 14 个用例：upload_screenshot 成功返回 URL / 失败缓存 / 异常缓存；report_audit_log 成功 / 失败缓存 / 异常缓存；report_step 完整闭环（前后截图+元数据全成功）/ 截图失败但元数据继续（URL 为 None）/ 无截图跳过上传直接报元数据；flush_cache 空缓存返回 0 / audit_log 成功删文件 / screenshot 成功删文件 / 失败保留文件待下次 / 混合类型（1 截图成功 + 1 审计日志失败） |
...
| **规模** | M |
| **前置依赖** | M7.1 |
| **产出物** | `audit/export/CsvExporter.java` + 检索增强 + 导出端点 |
| **描述** | 1. CSV 导出：按查询条件导出审计日志<br>2. 多维检索：时间范围 / 任务 / 用户 / 部门 / 业务线 / 风险等级 / 操作类型<br>3. 分页 + 排序<br>4. API：`GET /api/v1/audit/logs/export` / `GET /api/v1/audit/logs`（增强 sortField/sortOrder） |
| **验收标准** | ✅ CSV 导出格式正确（UTF-8 BOM + RFC 4180 转义）；✅ 多维检索支持 6 字段白名单动态排序；✅ 分页准确；✅ 导出限制最大 10000 条防 OOM |
| **状态** | ✅ 已完成（2026-08-03）。最终落地：<br>(1) **`CsvExporter.java`**（`audit/export/`）：纯工具类（私有构造 + 全 static 方法），不引入第三方 CSV 库，使用 JDK 原生 `OutputStreamWriter` 实现 + RFC 4180 转义规则；输出格式约定 —— UTF-8 with BOM（Excel 中文不乱码）/ 字段顺序固定对齐系统设计 6.4.1 六大维度（23 列表头）/ 含逗号或双引号或换行的字段用双引号包裹且内部双引号转义为 `""` / null 输出为空字符串 / Timestamp 格式化为 `yyyy-MM-dd HH:mm:ss` / BigDecimal 用 `toPlainString` 避免科学计数法；核心方法 `export(List<AuditLogVO>, OutputStream)` 写入 BOM 后用 UTF-8 编码写表头 + 数据行<br>(2) **`AuditConstant` 扩展**：新增 CSV 导出常量（`EXPORT_MAX_ROWS=10000` / `EXPORT_FILE_NAME_PREFIX="audit_logs_"` / `EXPORT_CONTENT_TYPE="text/csv; charset=UTF-8"`）+ 排序字段白名单（6 字段：auditId / taskId / riskLevel / startedAt / durationMs / createTime，使用 `Set<String>` 不可变集合防止 SQL 注入）<br>(3) **`AuditLogService` 接口扩展**：新增 `exportAuditLogs(AuditLogQueryRequest)` 返回 `List<AuditLogVO>`（不分页，用于 CSV 导出）；`listAuditLogs` 文档说明支持 sortField/sortOrder 动态排序<br>(4) **`AuditLogServiceImpl` 实现扩展**：①抽取 `buildQueryWrapper(queryRequest)` 私有方法构建 9 维过滤条件（组织/任务/用户/部门/业务线/风险等级/操作类型/执行结果/时间范围），分页与导出复用 ②抽取 `applySort(wrapper, sortField, sortOrder)` 私有方法应用动态排序 —— sortOrder 非 "ascend" 一律按降序处理，sortField 为空或不在白名单内时回退默认按 createTime 倒序（与 M7.1 行为一致），白名单字段用 switch 分发到对应 `orderByAsc/Desc` ③`exportAuditLogs` 实现：buildQueryWrapper + applySort + `wrapper.last("LIMIT " + EXPORT_MAX_ROWS)` 限制最大条数防 OOM，selectList 后 stream map 转 VO<br>(5) **`AuditController` 扩展**：新增 `GET /v1/audit/logs/export` 端点（注意：放在 `/{auditId}` 之前避免 "export" 被当作 path variable 解析）—— 从 TenantContext 填充 orgId → 调 service 查询 → 设置响应头（`Content-Type: text/csv; charset=UTF-8` + `Content-Disposition: attachment; filename="audit_logs_yyyyMMdd.csv"` 文件名 URL 编码兼容中文）→ 用 `response.getOutputStream()` 写入 CsvExporter 生成的 CSV 字节流；文件名按日期生成（`audit_logs_yyyyMMdd.csv`）；`buildExportFileName()` 私有方法<br>(6) **API 路径**：按 requirements.md 6.5 节定义为 `GET /api/v1/audit/logs/export`（不是 task-breakdown.md 原始描述的 `/api/v1/audit/export`，以 requirements.md 为准） |
| **测试覆盖** | ✅ 37 个单元测试全部通过（547 total）：(1) **`CsvExporterTest`** 11 个用例：空列表仅含 BOM + 表头 / 单条完整记录所有字段正确输出 / 多条记录按顺序输出 / 含逗号字段用双引号包裹 / 含双引号字段转义为两个双引号并包裹 / 含换行字段用双引号包裹 / null 字段输出为空字符串 / Timestamp 格式化正确 / BigDecimal 用 toPlainString / null logs 抛 NPE / null OutputStream 抛 NPE (2) **`AuditLogServiceImplTest`** 扩展至 19 个用例（原 11 + 新增 8）：listAuditLogs 携带 sortField=startedAt+sortOrder=ascend 正常调用 / sortField 非法（含 SQL 注入字符串）时回退默认排序不抛异常 / sortOrder 为 null 时按降序处理；exportAuditLogs 全量查询返回 VO 列表 / 空结果返回空列表 / selectList 返回 null 时返回空列表 / 带 sortField 时正常查询 (3) **`AuditControllerTest`** 扩展至 7 个用例（原 3 + 新增 4）：listAuditLogs 携带 sortField/sortOrder 参数正常透传到 service；CSV 导出成功返回 text/csv + Content-Disposition + 表头 + 数据 / 空结果仅含表头 / 无登录态时仍可导出 |
...
| **规模** | L |
| **前置依赖** | M7.1、M7.4 |
| **产出物** | `routes/enterprise/AuditLogs.tsx` + `api/audit.ts` + `mock/mockServer.ts` 扩展 + `styles/glass.css` 扩展 + 路由/导航接入 |
| **描述** | 1. 日志列表：多维筛选 + 分页<br>2. 详情：截图对比（before/after）+ 操作参数 + LLM 信息<br>3. 导出按钮：触发 CSV 下载<br>4. 时间线视图：按任务维度展示操作时间线 |
| **验收标准** | ✅ 列表筛选正确；✅ 详情截图可查看；✅ 导出功能可用；✅ 时间线视图按任务聚合展示操作时间线 |
| **状态** | ✅ 已完成（2026-08-03）。最终落地：<br>(1) **`routes/enterprise/AuditLogs.tsx`**（主页面，约 1100 行）：双 Tab 视图（列表视图 / 时间线视图），共享筛选栏；列表视图含 10 列表格（审计 ID / 任务 ID / 操作类型 / 执行结果 / 风险等级 / 开始时间 / 耗时 / LLM / 页面 / 操作），分页器复用 `Pagination` 组件；时间线视图按 taskId 聚合（Map 分组 + 按 startedAt 升序），每组展示步数 / 成功率 / 累计耗时 / 时间范围 + 可折叠时间线条目（步骤序号 + 操作类型 + 风险标签 + 结果标签 + LLM 信息 + 时间 + 耗时 + 页面 URL）；详情弹窗（`AuditDetailModal`）展示基本信息网格（12 字段，含审批单 ID / 用户 ID / 部门 ID）+ 页面 URL 链接 + 目标元素 + 错误信息（失败时）+ 截图对比（before/after 双列 grid，无图占位符，MinIO 预签名 URL 时效提示）+ 操作参数 JSON 美化展示（已脱敏标注）+ LLM 信息网格（模型 / Token 用量 / 成本 USD）；CSV 导出按钮调用 `auditApi.exportAuditLogs` 触发 Blob 下载；筛选条件 6 维（任务 ID / 风险等级 / 操作类型 / 执行结果 / 开始时间起止）+ 排序字段 6 选（createTime / startedAt / durationMs / riskLevel / auditId / taskId）+ 排序顺序<br>(2) **`api/audit.ts`**：3 个 API 封装 —— `listAuditLogs`（GET /v1/audit/logs，使用 axiosClient 业务码拦截，9 维查询参数透传）、`getAuditLogDetail`（GET /v1/audit/logs/{auditId}）、`exportAuditLogs`（GET /v1/audit/logs/export，绕过 axiosClient 用原始 axios + responseType:'blob' 接收二进制流，从 Content-Disposition 解析文件名 RFC 5987 / 普通 filename 双格式兼容，Blob URL + 模拟 a 标签点击触发下载，失败回退默认文件名 `audit_logs_yyyyMMdd.csv`）；导出 `auditApi` 聚合对象<br>(3) **`api/types.ts` 扩展**：新增 6 个类型 —— `AuditLogVO`（23 字段对齐后端 VO，含 beforeScreenshotUrl/afterScreenshotUrl/llmModel/llmTokensUsed/llmCost）、`AuditLogQueryRequest`（分页 + 排序 + 9 维筛选）、`AuditActionType`（8 种操作：NAVIGATE/CLICK/INPUT_TEXT/LOGIN/FILE_DOWNLOAD/FORM_FILL/WAIT/SCREENSHOT）、`AuditExecutionResult`（success/failed）、`AuditSortField`（6 字段）、`SortOrder`（ascend/descend）<br>(4) **`mock/mockServer.ts` 扩展**：新增 MockAuditLog 接口 + 10 条 mock 审计日志数据（覆盖 success/failed 两种结果 + low/medium/high/critical 四种风险等级 + 7 种操作类型 + LLM 信息 + 截图 URL）；新增 3 个 handler —— `handleListAuditLogs`（支持 9 维过滤 + 6 字段排序 + 分页，返回 IPage 结构）、`handleGetAuditLogDetail`（按 auditId 查找单条）、`handleExportAuditLogs`（生成 CSV 字符串 + UTF-8 BOM + RFC 4180 转义，返回 text/csv 二进制流 + Content-Disposition 头）<br>(5) **`styles/glass.css` 扩展**：新增审计页面样式 —— `.audit-toolbar`（筛选栏 grid 布局）/ `.audit-table`（表格列宽优化）/ `.audit-timeline-container`（时间线视图容器）/ `.audit-timeline-group`（任务分组卡片）/ `.audit-timeline-list`（时间线列表 + 左侧竖线连接）/ `.audit-timeline-dot`（步骤圆点 + 成功/失败/风险等级配色）/ `.audit-modal-card`（详情弹窗宽高度）/ `.audit-screenshot-grid`（截图双列对比）/ `.audit-screenshot-placeholder`（无截图占位）/ `.audit-llm-grid`（LLM 信息自适应网格）/ `.audit-export-toast`（导出状态悬浮提示）<br>(6) **路由与导航接入**：`router.tsx` 新增 `/audit-logs` 路由（lazy import）；`RootLayout.tsx` 顶部导航新增"审计日志"按钮（IconShield 图标）；`Icons.tsx` 新增 `IconDownload` 图标用于导出按钮<br>(7) **类型检查与构建**：`tsc --noEmit` 通过，`vite build` 成功（806 模块，5.14s）<br>(8) **浏览器验证**：内置浏览器登录后访问 `/audit-logs`，验证列表视图（10 条 mock 数据 + 10 列表格 + 筛选栏 + 排序）、时间线视图（按 taskId 分组 + 步数/成功率/耗时统计 + 可折叠）、详情弹窗（12 字段基本信息 + 截图对比 + 操作参数 JSON + LLM 信息）、筛选功能（风险等级=高 过滤生效）、CSV 导出功能全部正常 |
...
| **状态** | ✅ 套件已创建（2026-08-06）。5 个场景测试脚本 + 百万造数脚本 + 性能统计工具（P50/P95/P99/throughput）+ 报告模板。**策略**：(1) 并发压测混合策略——10 并发用真实 Skyvern 验证端到端，50/100 并发用内部回调模拟（绕过 Skyvern 视觉决策，专注测 Java+PG 并发能力）；(2) 审计百万造数——直连 PG 用 `generate_series` 批量 INSERT，`audit_id >= 8000000000000000000`（低于 bigint 上限 9.22×10^18）避开雪花 ID 范围，测试后自动清理 + ANALYZE；(3) LLM 成本基线——真实调用火山方舟豆包 5 次风险判断，从 `rpa_llm_call_log` 表统计 token/cost。**注**：测试结果待全栈启动后执行 `npm test` 填充 `docs/perf-test-report.md`。**场景3 SSE 推送延迟已通过（2026-08-06）**：P95=3ms（< 500ms ✓），首个事件延迟 5996ms（< 60000ms ✓），共 43 个事件（42 progress + 1 complete），耗时 3.8min。执行中发现并修复 2 个真实 bug：① Python SSE timestamp 时区偏移——`event_bus.py`/`schemas.py`/`resilient_caller.py` 3 处 `datetime.utcnow()` 生成无时区 ISO 字符串，前端 `new Date()` 按本地时区解析造成 8 小时偏移（P95 从 28800003ms 降至 3ms）；② Skyvern 监控路径不发中间 SSE 事件——`tasks.py` `_monitor_skyvern_task` 仅在终态发一次 complete，中间轮询无 progress，首个事件延迟=任务执行时长（234711ms），修复后轮询循环非终态时发布 progress 事件，首个延迟降至 5996ms（同时改善前端任务详情页进度反馈体验）。**场景2 改为方案B混合策略（2026-08-06）**：第1组 5 并发真实 Skyvern（Playwright，从 10 降为 5 避免本地资源压力）+ 第2/3组 50/100 并发模拟回调改用 JMeter（`jmeter/concurrent-mock.jmx`，参数化 `concurrent.level`，生成 HTML 报告）；新增 `package.json` 脚本 `test:concurrent:jmx:50/100`。**场景2 并发吞吐量已通过（2026-08-06）**：第1组 5 并发真实 Skyvern 成功率 100%（P50=299s/P95=357s/6min）；第2组 50 并发 JMeter 0 错误（P50=9ms/P95=67ms/17 req/s）；第3组 100 并发 JMeter 0 错误（P50=7.5ms/P95=70ms/33 req/s）；瓶颈分析：端到端延迟瓶颈在 Skyvern LLM 视觉决策，Java 后端 100 并发 P95<100ms 余量充足。**场景1 单任务延迟基线已通过（2026-08-06）**：触发 1502ms（< 5s ✓）/ 详情 88ms（< 1s ✓）/ 端到端 5.4min（基线）/ 审计 32ms（< 500ms ✓）。**场景4 审计查询性能已通过（2026-08-06）**：100 万条造数，4 维度 P95 均达标——分页查询 37ms / 多维检索 21ms / 深度翻页第1000页 15ms / COUNT 查询 146ms（均 < 500ms，深度翻页 < 2000ms），耗时 8.9s（造数 19.2s）。执行中发现并修复 5 个测试侧 bug：① 造数 audit_id 起始值 9.9e18 越界 bigint 上限→改为 8e18；② 造数 SQL 数组下标 `$3[...]` 类型推断失败→加括号显式 cast `($3::text[])[...]`；③ 造数 `float::int` 四舍五入致数组越界返回 NULL 违反 NOT NULL→改用 `floor()` 截断；④ 测试多维检索传 ISO 8601 带 Z 字符串，后端 `java.sql.Timestamp` 无法解析返回 403→改传 `yyyy-MM-dd HH:mm:ss`；⑤ 全部失败时 P95=0 误报通过延迟断言→增加成功率断言 |
...
| **状态** | ✅ 已完成（2026-08-06）。6 大产出全部交付：(1) **HTTPS 启用**：`nginx/conf.d/prod.conf` 新增 443 端口 HTTPS server 块（TLSv1.2/1.3 + 现代加密套件 + 会话缓存），HTTP 80 端口 301 重定向到 HTTPS，6 个安全头（HSTS/X-Frame-Options/X-Content-Type-Options/X-XSS-Protection/Referrer-Policy/CSP），`http2 on` 指令（nginx 1.25+ 语法，避免 `listen ... http2` 弃用警告）；SSL 证书挂载 `./nginx/ssl:/etc/nginx/ssl:ro`，`.gitignore` 忽略证书文件。(2) **资源限制**：7 个服务全部配置 `deploy.resources.limits`（CPU+内存）+ `reservations`（内存），finance-ai 4G/2CPU（Skyvern+Chromium），finance-backend 1G/1CPU（M9.2 测试 100 并发 P95<100ms 余量充足），postgres 1G/1CPU，redis 256M/0.5CPU（启用 maxmemory 256mb + LRU），minio 512M/0.5CPU，frontend 128M/0.25CPU，nginx 256M/0.5CPU。(3) **日志卷挂载**：`backend-logs`/`ai-logs`/`nginx-logs` 独立命名卷，backend 日志按天滚动 30 天（logback rollingpolicy），`ai-data`+`ai-logs` 分离 Skyvern 数据与日志。(4) **健康检查强化**：所有服务配置 `healthcheck`（interval/timeout/retries/start_period），finance-ai start_period=300s（uv sync+alembic+playwright 初始化），redis 健康检查带密码认证，`depends_on: condition: service_healthy` 强制启动顺序。(5) **备份策略**：`pg-backup`/`minio-backup` 命名卷预挂载，部署文档给出 `pg_dump`/`mc mirror` 手动命令 + crontab 自动备份示例（M9.4 将封装为 `scripts/backup.sh`）。(6) **部署文档** `docs/deployment.md` 9 章：硬件/软件要求、3 种 HTTPS 证书方案（自签/Let's Encrypt/商业）、必填变量清单+一键生成命令、一键启动+验证、服务架构+资源限制+健康检查策略+日志卷、备份策略+手动命令+自动 crontab、7 类故障排查、升级与回滚、安全清单、速查附录。**安全加固**：数据层端口仅 `expose` 内网（不 `ports` 暴露），仅 nginx 80/443 + MinIO Console 暴露；actuator `show-details: never` 不暴露内部组件状态；`application-prod.yml` 关闭 SQL 日志（Slf4jImpl）+ 调整日志级别（com.finrpa: INFO）。**验证**：`docker-compose --env-file .env.prod.example -f docker-compose.prod.yml config` 输出 9739 字节配置（所有 `${VAR:?...}` 必填变量解析成功）；`nginx -t` 语法检查通过（`syntax is ok` + `test is successful`，无 warning）；application-prod.yml YAML 语法通过 docker-compose 解析。**配套产出**：`.env.prod.example`（11 个必填变量+8 个可选变量）、`nginx/ssl/.gitkeep`（证书目录占位）、`.gitignore` 新增 `.env.prod` + `nginx/ssl/*.{crt,key,pem}` |
...
|----|------|
| **规模** | M |
| **前置依赖** | M5.1、M5.2、M5.3、M9.2 |
| **产出物** | `app/llm/llm_key_mapper.py` + `agent.py` / `api_handler_factory.py` / `tasks.py` 改造 |
| **描述** | 1. **侵入点调研**：4 个模块接口（api_handler_factory / agent / tasks / llm_key_mapper）+ Skyvern LLM 调用入口（`Agent.execute_step` 内 `llm_api_handler` 调用点）<br>2. **接入方案**：在 Skyvern 原生 LLM 调用链 4 个侵入点注入 M5 系列能力 — ① litellm 调用后上报 rpa_llm_call_log；② trigger_task 改走 Coordinator + action_handler + ResilientCaller；③ 模型名→llm_key 映射；④ ActionCache 查缓存 + ModelRouter 动态选模型<br>3. **顺序**：先接 api_handler_factory（最低风险，仅上报）→ 再接 tasks.py（编排链路切换）→ 最后接 agent.py（缓存 + 路由，影响 LLM 调用本身）<br>4. **风险**：Skyvern 源码侵入导致升级困难（用 try/except 包裹降级）、跨 provider llm_key 错误覆盖、Redis 连接泄漏 |
| **验收标准** | 4 个侵入点全部接入；语法校验通过；Docker 重建后场景1 端到端延迟较优化前下降 |
| **状态** | ✅ 已完成（2026-08-06）。完成 4 个侵入点接入 + 3 个 HIGH/MEDIUM 质量问题修复 + 场景1 性能验证。最终落地：<br>(1) **新建 `app/llm/llm_key_mapper.py`**：`route_by_complexity(level, original_llm_key)` 函数 + `_PROVIDER_COMPLEXITY_ROUTING` provider 感知路由表（OPENAI: light→OPENAI_GPT4O_MINI / standard→OPENAI_GPT4O / heavy→OPENAI_GPT-4O-2024-08-06；VOLCENGINE: light→VOLCENGINE_DOUBAO_SEED_1_6_FLASH）；按 `original_llm_key.startswith(provider_prefix)` 判定 provider，仅在同 provider 内路由，未匹配时原样返回<br>(2) **改 `skyvern/forge/sdk/api/llm/api_handler_factory.py`**：litellm 调用成功后构造 `LlmCallRecord`（task_id=step.task_id / org_id=step.organization_id / model=llm_config.model_name / prompt_tokens / completion_tokens / total_tokens / duration_ms / timestamp 带时区）→ `JavaBackendClient.report_llm_call()` 上报 Java `rpa_llm_call_log`；try/except 包裹失败仅 warning 不阻断 Skyvern 主流程<br>(3) **改 `app/api/tasks.py` `trigger_task`**：从直接调 `SkyvernClient` 改走 `AgentCoordinator` 编排 — 构造 `ResilientCaller`（三层容错 + NEEDS_HUMAN 兜底）+ 内嵌 `_skyvern_action_handler(goal, context)` 闭包（调 `SkyvernClient.create_task` + 轮询终态）→ `AgentCoordinator(planner, executor, action_handler=...)` 启动后台编排；Planner 拆任务 / Executor 逐子任务调 action_handler / 失败策略 / 断点续跑 / 子任务级审计全链路打通<br>(4) **改 `skyvern/forge/agent.py` `execute_step`**：在 `llm_api_handler` 调用前插入 ModelRouter 动态选模型（`ModelRouter.route(html)` 评分 → `route_by_complexity(level, llm_key_override)` 同 provider 路由）+ ActionCache 查缓存（`ActionCache.get_instance().get(html, navigation_goal)` 命中跳过 LLM 调用并补报 cache_hit 记录 / 未命中调 LLM 后写缓存 TTL 24h）；try/except 包裹 ModelRouter / ActionCache 异常均降级到原生 LLM 调用<br>(5) **修 HIGH：Redis 连接泄漏** — `ActionCache` 改单例模式（`_instance` + `get_instance()` 类方法），`agent.py` 调用点改用 `ActionCache.get_instance()` 复用全局连接池，避免每个 step 创建新 Redis 连接池导致泄漏<br>(6) **修 HIGH：跨 provider llm_key 错误覆盖** — `llm_key_mapper.py` 新增 provider 感知路由，原 `map_model_to_llm_key` 即使 original_llm_key 是 VOLCENGINE 也映射到 OPENAI key 造成 API key 不匹配，现按 provider_prefix 同 provider 内路由<br>(7) **修 MEDIUM：LLM 调用记录缺 task_id/org_id** — `api_handler_factory.py` 的 `LlmCallRecord` 原本无 task_id/org_id，无法按任务维度做成本分析，现从 `step.task_id` / `step.organization_id` 填充<br>(8) **场景1 性能验证**：重跑 `tests/perf/scenarios/01-single-task-latency.spec.ts`，端到端延迟从优化前 321246ms（5.4min）降至 187585ms（3.1min），降幅约 41.6%；触发延迟 / 任务详情查询 / 审计日志查询均维持低位（详见 `docs/perf-test-report.md` 3.1 节） |
| **测试覆盖** | Python 语法校验通过（`python -m py_compile`）；Docker compose 重建 finance-ai 容器热重载成功；场景1 单任务延迟测试 4/4 通过（successRate=1，详见 `tests/perf/perf-single-task.perf.json`） |
