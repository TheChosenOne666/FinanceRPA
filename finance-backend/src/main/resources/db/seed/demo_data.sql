...
SELECT 100, 'banking_admin', '$2b$10$8XsgDVGRtvGl.9Bj5eoR7.XCAbRDCtDQdy.DU66AV/3IVkpS4qqWu', '平台管理员', 1, '招商银行演示', '信息技术部', 1, 0
WHERE NOT EXISTS (SELECT 1 FROM sys_user WHERE username = 'banking_admin');

INSERT INTO sys_user (user_id, username, password, real_name, org_id, org_name, dept_name, status, deleted)
SELECT 101, 'credit_operator', '$2b$10$8XsgDVGRtvGl.9Bj5eoR7.XCAbRDCtDQdy.DU66AV/3IVkpS4qqWu', '对公信贷操作员', 1, '招商银行演示', '对公信贷部', 1, 0
WHERE NOT EXISTS (SELECT 1 FROM sys_user WHERE username = 'credit_operator');

INSERT INTO sys_user (user_id, username, password, real_name, org_id, org_name, dept_name, status, deleted)
SELECT 102, 'credit_approver', '$2b$10$8XsgDVGRtvGl.9Bj5eoR7.XCAbRDCtDQdy.DU66AV/3IVkpS4qqWu', '对公信贷审批员', 1, '招商银行演示', '对公信贷部', 1, 0
WHERE NOT EXISTS (SELECT 1 FROM sys_user WHERE username = 'credit_approver');

INSERT INTO sys_user (user_id, username, password, real_name, org_id, org_name, dept_name, status, deleted)
SELECT 103, 'risk_viewer', '$2b$10$8XsgDVGRtvGl.9Bj5eoR7.XCAbRDCtDQdy.DU66AV/3IVkpS4qqWu', '风险管理员', 1, '招商银行演示', '风险管理部', 1, 0
WHERE NOT EXISTS (SELECT 1 FROM sys_user WHERE username = 'risk_viewer');

INSERT INTO sys_user (user_id, username, password, real_name, org_id, org_name, dept_name, status, deleted)
SELECT 104, 'compliance_approver', '$2b$10$8XsgDVGRtvGl.9Bj5eoR7.XCAbRDCtDQdy.DU66AV/3IVkpS4qqWu', '合规审计员', 1, '招商银行演示', '合规审计部', 1, 0
